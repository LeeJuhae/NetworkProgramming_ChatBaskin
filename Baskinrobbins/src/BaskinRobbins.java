...
public class BaskinRobbins extends JFrame {
	...
	static SSLServerSocket server = null;
	static SSLSocket baskinSocket = null;
	...
	public void connect(String eserver, int eport) {
		...
		try {
			System.setProperty("javax.net.ssl.trustStore",
					"C://Users//samsung//Desktop//Baskinrobbins//bin//trustedcerts");
			System.setProperty("javax.net.ssl.trustPassword", "123456");
			BaskinSocketFactory bf = new BaskinSocketFactory("TLS");
			baskinSocket = (SSLSocket)bf.createSocket(eServer,ePort);
			String[] supported = baskinSocket.getSupportedCipherSuites();
			baskinSocket.setEnabledCipherSuites(supported);
			baskinSocket.startHandshake();
...
class ClientSender {
	private SSLSocket baskinSocket = null;
	private BaskinRobbins baskin = null;

	String userInput = "";
	PrintWriter out = null;

	ClientSender(SSLSocket socket, BaskinRobbins baskin) {
...
class ClientReceiver implements Runnable {
	...
	ClientReceiver(SSLSocket socket, BaskinRobbins baskin) {
		this.baskinSocket = socket;
		this.baskin = baskin;
	}
			...
			try {
				...
				while ((readSome = in.readLine()) != null) {
					if (readSome.contains("Connected")) {
						...
						baskin.jpanel4.numberArea.setCaretPosition(baskin.jpanel4.numberArea.getDocument().getLength());
						baskin.jpanel4.lblNewLabel_1.setVisible(false);
						baskin.jpanel4.lblNewLabel_2.setVisible(false);
						baskin.jpanel4.lblNewLabel_3.setVisible(false);
						baskin.jpanel4.lblNewLabel_4.setVisible(false);
